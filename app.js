...
// ==========================================
// MÁGICA DO ARREDONDAMENTO E TAXAS SHOPEE/ML
// ==========================================
function descontarTaxas(valorBruto, qtdTotal, cartItemsArray) { 
    var feeShpTotal = 0, feeMlTotal = 0, txMl = Number(pegaValor('taxaMeli')) / 100;
    ...
    var vBrutoNum = Number(parseLocal(valorBruto));
    
    if (isCart) {
        var cLog = Number(pegaValor('custoEmbalagem')) + Number(pegaValor('custoDeslocamento'));
        var totBaseForRatio = items.reduce((a,b) => a + Number(parseLocal(b.valorComLucro || 0)), 0) || 1;
        var simulatedGrossList = [], totalSimulatedGross = 0;
        
        items.forEach(i => {
            var iQtd = Number(parseLocal(i.qtd || 1));
            var iValLucro = Number(parseLocal(i.valorComLucro || 0));
            var iPrecoExato = Number(parseLocal(i.precoVendaExato || 0));
            ...
            if (iPrecoExato > 0) {
                ...
                var p1 = (itemBaseUnit + 4) / 0.80;
                var p2 = (itemBaseUnit + 16) / 0.86;
                var p3 = (itemBaseUnit + 20) / 0.86;
                var p4 = (itemBaseUnit + 26) / 0.86;
                var bestPShp;
                
                if (p1 <= 79.991) bestPShp = p1; 
                else if (p2 <= 99.991) bestPShp = p2; 
                else if (p3 <= 199.991) bestPShp = p3; 
                else bestPShp = p4;
        ...
        items.forEach((i, idx) => {
            var iQtd = Number(parseLocal(i.qtd || 1));
            var actualUnitGross = simulatedGrossList[idx] * scale;
            
            var feeSUnit = 0;
            if (actualUnitGross <= 79.991) feeSUnit = (actualUnitGross * 0.20) + 4;
            else if (actualUnitGross <= 99.991) feeSUnit = (actualUnitGross * 0.14) + 16;
            else if (actualUnitGross <= 199.991) feeSUnit = (actualUnitGross * 0.14) + 20;
            else feeSUnit = (actualUnitGross * 0.14) + 26;
            
            feeShpTotal += (feeSUnit * iQtd);
            
            var fixMl = (actualUnitGross >= 79.99) ? 0 : Number(pegaValor('fixaMeli'));
            var feeMUnit = (actualUnitGross * txMl) + fixMl;
            feeMlTotal += (feeMUnit * iQtd);
        });
    } else {
        var qTotalLocal = Number(parseLocal(qtdTotal));
        if (qTotalLocal < 1) qTotalLocal = 1;
        var avgBruto = vBrutoNum / qTotalLocal;
        ...
        var fixMl = (avgBruto >= 79.99) ? 0 : Number(pegaValor('fixaMeli'));
...
function renderCarrinho() {
    ...
    carrinho.forEach(item => { 
        totCusto += Number(parseLocal(item.custo)); totValorComLucro += Number(parseLocal(item.valorComLucro)); 
    ...
    var totalQtd = carrinho.reduce((a,b) => a + Number(parseLocal(b.qtd)), 0); if(totalQtd < 1) totalQtd = 1; 
    var cLog = Number(pegaValor('custoEmbalagem')) + Number(pegaValor('custoDeslocamento')); 
    var frete = Number(pegaValor('valorFreteManual')); 
    
    var totS = 0, totM = 0, totD = 0, totBaseForRatio = totValorComLucro === 0 ? 1 : totValorComLucro;
    
    carrinho.forEach(i => {
        var iQtd = Number(parseLocal(i.qtd || 1)), iPrecoExato = Number(parseLocal(i.precoVendaExato || 0)), iValLucro = Number(parseLocal(i.valorComLucro || 0));
        if (iPrecoExato > 0) {
            ...
            var txMl = Number(pegaValor('taxaMeli'))/100, pAvgML_noFix = itemBaseUnit / (1 - txMl);
            var bestPMeli = (pAvgML_noFix >= 79.99) ? pAvgML_noFix : (itemBaseUnit + Number(pegaValor('fixaMeli'))) / (1 - txMl);
...
function renderHistorico() {
    ...
    itensFiltrados.forEach(function(item, index) {
        ...
        // === LÓGICA DO CONTAGEM REGRESSIVA (PRAZO) ===
        var countdownHtml = "";
