if ('serviceWorker' in navigator) { window.addEventListener('load', () => { navigator.serviceWorker.register('./sw.js').then(reg => console.log('PWA Service Worker Registrado!')).catch(err => console.log('Erro no Service Worker', err)); }); }
...
// --- UPLOAD IMGBB NO CATÁLOGO ---
window.handleUploadCat = function(input) {
    var file = input.files[0]; if (!file) return;
    var formData = new FormData(); formData.append("image", file);
    document.getElementById('loading-screen').style.display = 'flex';
    var h2 = document.getElementById('loading-screen').querySelector('h2'), oldText = h2.textContent; h2.textContent = "A anexar foto...";
    ...
    .then(r => r.json()).then(data => {
        ...
        if (data.success) {
            ...
            prev.style.backgroundImage = `url('${url}')`; prev.style.display = "block";
...
window.removerFotoCat = function() {
    document.getElementById('fotoUrlCat').value = "";
    document.getElementById('previewFotoCat').style.display = "none";
...
function iniciarApp() {
    unsubscribeSnapshot = nuvemRef.onSnapshot(function(doc) {
        if (doc.exists) {
            ...
            let elMargemSlider = document.getElementById('margemSlider'); if(elMargemSlider) { elMargemSlider.value = pegaValor('margemInput'); updateSliderProgress(elMargemSlider); }
            let precisaSalvarNuven = false;
            historico.forEach(h => { if ((h.custo === 0 || isNaN(h.custo)) && h.status !== 'Orçamento' && h.status !== 'Devolução') { let hrs = parseLocal(h.tempo) || 0, grm = parseLocal(h.peso) || 0; if (hrs > 0 || grm > 0) { let dep = (pegaValor('maquina') / (pegaValor('vidaUtil') || 1)) * hrs, ene = (pegaValor('consumoW') / 1000) * pegaValor('precoKwh') * hrs, mat = (120 / 1000) * grm; h.custo = dep + ene + mat; precisaSalvarNuven = true; } } });
            if(precisaSalvarNuven) syncNuvem(); renderHistorico(); renderEstoque(); renderCatalogo(); renderDespesas(); atualizarDropdownsEstoque(); calcular(); 
...
window.fecharModal = function(idModal) {
    ...
    if (idModal === 'logisticaModal') {
        ...
        if (nEmb !== oldEmb || nDes !== oldDes) { if(confirm("⚠️ Você alterou os custos Fixos de Logística.\n\n[ OK ] Atualiza TODAS as vendas já feitas no histórico.\n[ CANCELAR ] O novo custo valerá apenas a partir de agora.")) { historico.forEach(h => { var q = h.totalQtd || 1, novoLog = (nEmb * q) + nDes, deltaLogistica = novoLog - parseLocal(h.logistica || 0); h.logistica = novoLog; h.valorLiquido = (parseLocal(h.valorLiquido) || 0) - deltaLogistica; if (h.valorLiquido < 0) h.valorLiquido = 0; }); showToast("✅ Logística Atualizada no Histórico!"); } }
    }
...
function resetarInputProjeto() {
    ...
    var sel1 = document.getElementById('sel_est_1'); if(sel1) sel1.value = ""; document.getElementById('tipoFilamento1').value = ""; document.getElementById('corFilamento1').value = ""; document.getElementById('marcaFilamento1').value = ""; document.getElementById('precoFilamento').value = "120,00"; document.getElementById('detalhes_1').style.display = 'none';
...
function preencherFormProjeto(prod) {
    ...
    document.getElementById('nomeProjeto').value = baseNome; document.getElementById('qtdPecasProjeto').value = qtd;
    ...
    document.getElementById('tempoH').value = prod.tempo1 || (prod.tempo ? formatarMoeda((prod.tempo) / qtd) : (matchCat ? matchCat.tempo : "")); document.getElementById('pesoPeca').value = prod.peso1 || (prod.peso ? formatarMoeda((prod.peso) / qtd) : (matchCat ? matchCat.peso1 : ""));
    if (prod.taxaSucesso) document.getElementById('taxaSucesso').value = prod.taxaSucesso;
    if (prod.margemLucro) { document.getElementById('margemInput').value = prod.margemLucro; document.getElementById('margemSlider').value = prod.margemLucro; updateSliderProgress(document.getElementById('margemSlider')); }
    document.getElementById('tipoFilamento1').value = p_tipo1; document.getElementById('corFilamento1').value = p_cor1; document.getElementById('marcaFilamento1').value = p_marca1; document.getElementById('precoFilamento').value = p_preco1;
    ...
    // CARREGAR A FOTO (PREVIEW MAIN)
    var fotoUrl = prod.foto || (matchCat ? matchCat.foto : "");
    document.getElementById('fotoUrlProjeto').value = fotoUrl; salvarDinamico('fotoUrlProjeto');
    var preview = document.getElementById('previewFotoMain'); 
    if (fotoUrl) { preview.style.backgroundImage = `url('${fotoUrl}')`; preview.style.display = "block"; } 
    ...
    if(p_multi && p_extras && p_extras.length > 0) {
        ...
        p_extras.forEach((ex, idx) => { var i = idx + 2; salvarDinamicoValor('tipoFilamento'+i, ex.tipo || ""); salvarDinamicoValor('corFilamento'+i, ex.cor || ""); salvarDinamicoValor('marcaFilamento'+i, ex.marca || ""); salvarDinamicoValor('precoFilamento'+i, ex.preco || "120,00"); salvarDinamicoValor('pesoPeca'+i, ex.peso || ""); });
        renderCoresExtras();
    ...
    } else { if(document.getElementById('qtdCoresExtras')) document.getElementById('qtdCoresExtras').value = "1"; renderCoresExtras(); }
...
function adicionarAoCarrinho() {
    ...
    var tMulti = document.getElementById('toggle_multi_mat'), multiMatEnabled = tMulti ? tMulti.checked : false, tempoItem = pegaValor('tempoH') * qtdPecas, pesoItem = pegaValor('pesoPeca') * qtdPecas;
    var depCalc = (pegaValor('maquina') / (pegaValor('vidaUtil') || 1)) * tempoItem, eneCalc = (pegaValor('consumoW') / 1000) * pegaValor('precoKwh') * tempoItem, precoMatCalc = pegaValor('precoFilamento'); if(precoMatCalc === 0) precoMatCalc = 120;
    var matTotalCalc = (precoMatCalc / 1000) * pesoItem, materiaisArray = [], t1 = pegaTexto('tipoFilamento1'), c1 = pegaTexto('corFilamento1'), m1 = pegaTexto('marcaFilamento1'), nomeMat1 = (t1 + ' ' + c1 + ' ' + m1).trim();
    if (nomeMat1 === '') nomeMat1 = 'Filamento 1'; if(pesoItem > 0) materiaisArray.push(nomeMat1 + ' (' + pesoItem + 'g)');
    var extras = [];
...
function editarItemCarrinho(id) { var item = carrinho.find(i => i.id === id); if(!item) return; preencherFormProjeto(item); editandoCarrinhoId = id; var btnAdd = document.getElementById('btn_add_carrinho'); var btnCancel = document.getElementById('btn_cancelar_edicao'); if(btnAdd) { btnAdd.textContent = "💾 Atualizar Item"; btnAdd.style.background = "var(--purple)"; } if(btnCancel) btnCancel.style.display = "block"; showToast("✏️ Item carregado para edição!"); var dash = document.querySelector('.dashboard'); if(dash) dash.scrollTo({ top: 0, behavior: 'smooth' }); window.scrollTo({ top: 0, behavior: 'smooth' }); }

function renderCarrinho() {
    var container = document.getElementById('carrinho_container'); var lista = document.getElementById('lista_itens_carrinho'); if(!container || !lista) return;
    if(carrinho.length === 0) { container.style.display = 'none'; return; }
    container.style.display = 'block'; lista.innerHTML = ''; var totCusto = 0, totValorComLucro = 0;
    ...
    var totD = totValorComLucro + cLog + frete, vShopeeBase = totValorComLucro + cLog, avgBase = vShopeeBase / totalQtd, pAvg1 = (avgBase + 4) / 0.80, pAvg2 = (avgBase + 16) / 0.86, pAvg3 = (avgBase + 20) / 0.86, pAvg4 = (avgBase + 26) / 0.86, bestPAvg; if (pAvg1 <= 79.991) bestPAvg = pAvg1; else if (pAvg2 <= 99.991) bestPAvg = pAvg2; else if (pAvg3 <= 199.991) bestPAvg = pAvg3; else bestPAvg = pAvg4;
    var totS = bestPAvg * totalQtd, avgBaseML = vShopeeBase / totalQtd, pAvgML_noFix = avgBaseML / (1 - (pegaValor('taxaMeli')/100)), pAvgML = (pAvgML_noFix >= 79.99) ? pAvgML_noFix : (avgBaseML + pegaValor('fixaMeli')) / (1 - (pegaValor('taxaMeli')/100)), totM = pAvgML * totalQtd;
...
function salvarNoCatalogo() {
    ...
    resetarDados(); syncNuvem(); renderCatalogo();
}
...
function carregarDoCatalogo() { 
    ...
    if(!id) { resetarInputProjeto(); document.getElementById('canalVendaSelecionado').value = "Direta"; document.getElementById('valorPersonalizado').value = ""; mostrarValorPersonalizado(); var boxPreco = document.getElementById('boxPrecoFixo'); if(boxPreco) boxPreco.style.display = 'none'; document.getElementById('precoFixoCatMain').value = ''; calcular(); return; } 
...
function aplicarDadosNoForm(id, isEditing = false) {
    ...
    if (p.precoFixo && parseFloat(p.precoFixo.replace('.','').replace(',','.')) > 0) { document.getElementById('canalVendaSelecionado').value = 'Personalizado'; document.getElementById('valorPersonalizado').value = p.precoFixo; salvarDinamico('valorPersonalizado'); mostrarValorPersonalizado(); } 
    else { document.getElementById('canalVendaSelecionado').value = 'Direta'; document.getElementById('valorPersonalizado').value = ''; salvarDinamico('valorPersonalizado'); mostrarValorPersonalizado(); }
    calcular();
}
...
window.onload = function() {
    ...
    if(savedFoto) { var p = document.getElementById('previewFotoMain'); if(p) { p.style.backgroundImage = `url('${savedFoto}')`; p.style.display = "block"; } }
...
function calcular() {
    ...
    var cLog = (pegaValor('custoEmbalagem') * totalQtd) + pegaValor('custoDeslocamento'), frete = pegaValor('valorFreteManual'), divResumoLog = document.getElementById('resumo_logistica');
    ...
    if (isCart) { var totCartCusto = carrinho.reduce((a,b)=>a+b.custo, 0), totCartVenda = carrinho.reduce((a,b)=>a+b.valorComLucro, 0); custoProducaoTotal = totCartCusto; vd = totCartVenda + cLog + frete; vShopeeBase = totCartVenda + cLog; } 
    else { custoProducaoTotal = custoProducao; vd = valorComLucro + cLog + frete; vShopeeBase = valorComLucro + cLog; }
    var avgBase = vShopeeBase / totalQtd, pAvg1 = (avgBase + 4) / 0.80, pAvg2 = (avgBase + 16) / 0.86, pAvg3 = (avgBase + 20) / 0.86, pAvg4 = (avgBase + 26) / 0.86, bestPAvg; if (pAvg1 <= 79.991) bestPAvg = pAvg1; else if (pAvg2 <= 99.991) bestPAvg = pAvg2; else if (pAvg3 <= 199.991) bestPAvg = pAvg3; else bestPAvg = pAvg4;
    var totS = bestPAvg * totalQtd, avgBaseML = vShopeeBase / totalQtd, pAvgML_noFix = avgBaseML / (1 - (pegaValor('taxaMeli')/100)), pAvgML = (pAvgML_noFix >= 79.99) ? pAvgML_noFix : (avgBaseML + pegaValor('fixaMeli')) / (1 - (pegaValor('taxaMeli')/100)), totM = pAvgML * totalQtd;
    ...
    var lucroD = vd - custoProducaoTotal - cLog - frete, netS = descontarTaxas(totS, totalQtd).shopee, lucroS = netS - custoProducaoTotal - cLog, netM = descontarTaxas(totM, totalQtd).meli, lucroM = netM - custoProducaoTotal - cLog;
...
function salvarHistorico() {
    ...
    if(isCart) {
        ...
        cLog = (pegaValor('custoEmbalagem') * totalQtd) + pegaValor('custoDeslocamento'); freteCalculado = pegaValor('valorFreteManual');
    } else {
    ...
    if(canal === "Shopee") { valorVendaFinal = net.shopee - freteCalculado - cLog; } else if(canal === "Meli") { valorVendaFinal = net.meli - freteCalculado - cLog; } else { valorVendaFinal = valorBruto - freteCalculado - cLog; }
...
function renderHistorico() {
    ...
    itensFiltrados.forEach(function(item, index) {
        ...
        var htmlFoto = item.foto ? `<div style="width:45px; height:45px; border-radius:6px; background-image:url('${item.foto}'); background-size:cover; background-position:center; margin-right:10px; border:1px solid var(--border); flex-shrink:0;"></div>` : '';
